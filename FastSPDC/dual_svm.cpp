...
double dual_svm::calculate_dual(Data& train_data) const {
	...
	for (int i = 0; i < train_data.n_sample; ++i) {//遍历一次所有数据
		for (int j = train_data.index[i]; j < train_data.index[i + 1]; ++j) {//遍历一个数据,嵌套的时候小心别混淆索引
			...
			////复合赋值操作，小心别写成直接赋值了.另外一定要小心,与SPDC,FastSPDC有细微差别，是+=，而不是-=
		}
	}
	...
	if (loss == "hinge") {
		for (int i = 0; i < train_data.n_sample; ++i) {
			sum += alpha[i] * train_data.Y[i];//+=，//注意与SPDC的和FastSPDC的不同
		}
	}
	else if (loss == "L2_svm") {
		for (int i = 0; i < train_data.n_sample; ++i) {
...
double dual_svm::calculate_dual_FastSPDC(Data& train_data) const {
	////for SPDC_r7, SPDC_r8 and FastSPDC
	...
	for (int i = 0; i < train_data.n_sample; ++i) {
		if (loss == "L2_svm") {
			...
			std::cout << "Unavailable loss type in dual_svm.cpp//calculate_dual_FastSPDC" << std::endl;
		}
	}
...
void dual_svm::fit_ASDCA_v1(Data& train_data) {
	...
	for (int epoch = 0; epoch < n_epoch; ++epoch)
	{
		...
		for (int i = 0; i < n_sample; ++i) {//遍历一次所有数据
			for (int j = train_data.index[i]; j < train_data.index[i + 1]; ++j) {//遍历一个数据,嵌套的时候小心别混淆索引
				w_prox[train_data.col[j]] += alpha[i] * train_data.X[j] / ((double)n_sample*(lambda + kappa));//符合赋值操作，小心别写成直接赋值了，另外和FastSPDC也有点区别。
			}
		}
		////初始化结束
...
void dual_svm::fit_FastSPDC(Data& train_data) {
	std::cout << "invoking FastSPDC algorithm..." << std::endl;
	int n_sample = train_data.n_sample, n_feature = train_data.n_feature;//特征维度已经扩展
	std::cout << "n_sample = " << n_sample << std::endl
	...
	for (int epoch = 0; epoch < n_epoch; ++epoch)
	{
		...
		double primal_val = calculate_primal(train_data);
		double dual_val = calculate_dual_FastSPDC(train_data);
		dual_gap = primal_val - dual_val;
...
void dual_svm::fit_FastSPDCs(Data& train_data) {
	std::cout << "invoking FastSPDC-s algorithm..." << std::endl;
	int n_sample = train_data.n_sample, n_feature = train_data.n_feature;//特征维度已经扩展
	std::cout << "n_sample = " << n_sample << "  n_feature + 1 = " << n_feature << endl;
	std::cout << " lambda = " << lambda << std::endl;
	...
	if (lambda > 4 / (n_sample*gamma)) {
		////是否满足调用条件
		std::cout << "Error in dual_svm.cpp/FastSPDC(): the lambda is not small enough!" << std::endl;
		return;
	}
	...
	for (int epoch = 0; epoch < n_epoch; ++epoch)
	{
		...
		double primal_val = calculate_primal(train_data);
		double dual_val = calculate_dual_FastSPDC(train_data);////应该调用哪一个函数呢
		dual_gap = primal_val - dual_val;
