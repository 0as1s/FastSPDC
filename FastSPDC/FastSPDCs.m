...
loss = 'smooth_hinge';
%loss = 'L2_svm';
%%%%% lambda should be relatively small in FastSPDC(SPDC_r8), ot herwise call SPDC_r7 
lambda = 1e-6;%%lambda要小于1e-6
tol = 1e-15;
EPOCH = 15;%%epoch 次数
n_epoch = EPOCH;
m = 2;
...
n_epoch=EPOCH*m/H;
tic;
%%%% value in dual_gap_FastSPDCs are absolute value 
[dual_gap_FastSPDCs,primal_val_FastSPDCs,dual_val_FastSPDCs] = Interface(file_name, loss, algorithm, lambda, tol, n_epoch, verbose);
time = toc;
fprintf('Time: %f seconds \n', time);
...
%% Draw Sub-Optimality
figure
semilogy(3:m:size(primal_val_sdca)-1, primal_val_sdca(4:m:size(primal_val_sdca))-Optimal_val, 'g-*');%size(dual_gap_sdca)=30,每两个epoch画一次
...
hold on,semilogy(3:H:size(primal_val_FastSPDCs)*H-1, primal_val_FastSPDCs(2:1:size(primal_val_FastSPDCs))-Optimal_val,'k-v'); %size(dual_gap_spdc_r8)=15
...
legend('SDCA','SPDC','FastSPDC-s','ASDCA','SVRG', 'Location','southwest');
...
%% Draw dual-gap
figure
semilogy(3:m:size(dual_gap_sdca)-1, dual_gap_sdca(4:m:size(dual_gap_sdca)), 'g-*');%size(dual_gap_sdca)=30,每两个epoch画一次
hold on,semilogy(3:m:size(dual_gap_sdca)-1, dual_gap_spdc(4:m:size(dual_gap_spdc)), 'm-square'); %size(dual_gap_spdc)=30
hold on,semilogy(3:H:size(dual_gap_FastSPDCs)*H-1, dual_gap_FastSPDCs(2:1:size(dual_gap_FastSPDCs)),'k-v'); %size(dual_gap_spdc_r8)=15
%hold on,semilogy(0:H:size(dual_gap_asdca)*H-1, dual_gap_asdca,'r-+');
hold on,semilogy(3:m:size(dual_gap_asdca)*m-1, dual_gap_asdca(2:1:size(dual_gap_asdca)),'r-+');
hold off

legend('SDCA','SPDC','FastSPDC-s','ASDCA', 'Location','southwest');
xlabel({'Number of passes of data'});
ylabel({'Dual Gap'});
%% Draw Primal Value 
% figure
